...
async function _runPolicy() {
  ...
  try {
    state.outcome = await apiGet('/api/v2/policy?' + params.toString());
    renderOutcome();
  } catch (e) {
    console.error(e);
  }
...
$('#export').onclick = () => {
  ...
  apiCsv('/api/cars.csv?' + params.toString());
};
...
async function openDetail(id) {
  ...
  const c = await apiGet('/api/cars/' + id);
  if (!c || c.error) { closeDetail(); return; }
  const fee = c.fee || {};
